...
public class SetBonusManager : MonoBehaviour
{
    ...
    /// </summary>
    [ContextMenu("Refresh Set Links")]
    public void RefreshSetLinks()
    {
        LoadEquipmentSetsFromResources();
        AutoLinkItemsToSets();
        UpdateSetBonuses();
    ...
    [ContextMenu("Debug Current Set Bonuses")]
    public void DebugCurrentSetBonuses()
    {
        Debug.Log("=== CURRENT SET BONUSES ===");

        if (currentSetBonuses.Count == 0)
        {
            Debug.Log("No active set bonuses");
            return;
        }
        ...
        foreach (var setBonus in currentSetBonuses)
        {
            ...
            Debug.Log($"Active bonuses: {setBonus.activeBonuses.Count}");

            foreach (var requirement in setBonus.activeBonuses)
            {
                Debug.Log($"  - {requirement.GetRequirementText()}");
                foreach (var effect in requirement.bonusEffects)
                {
                    Debug.Log($"    • {effect.GetBonusText()}");
                }
            }
        }
    }

    [ContextMenu("Debug All Equipment Sets")]
    public void DebugAllEquipmentSets()
    {
        Debug.Log("=== ALL EQUIPMENT SETS ===");

        if (allEquipmentSets.Count == 0)
        {
            Debug.Log("No equipment sets found");
            return;
        }
        ...
        foreach (var set in allEquipmentSets)
        {
            ...
            Debug.Log($"Set: {set.setName}");
            Debug.Log($"Items: {set.setItems.Count}");
            Debug.Log($"Valid: {set.IsValidSet()}");
    ...
    [ContextMenu("Force Update Set Bonuses")]
    public void DebugForceUpdate()
    {
        ForceUpdateSetBonuses();
    }
    #endregion
}